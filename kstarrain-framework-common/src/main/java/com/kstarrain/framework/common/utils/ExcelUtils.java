 ...
 * @description: Excel 工具类  请了解清楚大致用法之后再用，不然有坑
...
public class ExcelUtils {
    ...
    public static Excel create(Excel.Type excelType) {
        switch (excelType) {
            ...
            case LARGE_XLSX:
                return new Excel(new SXSSFWorkbook(new XSSFWorkbook(), 100), excelType);
            default:
    ...
    public static <T> List<T> readToBeans(Excel excel, Map<String, String> titlePropertyMap, Map<String, DateFormat> propertyDateFormatMap, Class<T> clazz) throws ReflectiveOperationException, ParseException {
        ...
        HashMap<Method, Integer> usefulSetMethodsColumnIndex = new HashMap<>();
        List<?> rowContent = excel.readRow(0);

        for (int i = 0; i < rowContent.size(); i++) {
            Object cellValue = rowContent.get(i);
            if (!(cellValue instanceof String) || StringUtils.isAnyBlank((String) cellValue, titlePropertyMap.get(cellValue))){
                continue;
            }

            String methodName = "set" + StringUtils.capitalize(titlePropertyMap.get(cellValue));
            ...
            usefulSetMethodsColumnIndex.put(method, i);
        }
        ...
        for (int i = 1; i <= lastRowNum; i++) {
            ...
            for (Method usefulSetMethod : usefulSetMethods) {
                Integer columnIndex = usefulSetMethodsColumnIndex.get(usefulSetMethod);
                if (columnIndex > rowContent.size() - 1){
                    emptyCellNum ++;
                    continue;
                }

                Object cellValue = rowContent.get(columnIndex);
                if (String.class.equals(cellValue.getClass()) && StringUtils.isBlank((String) cellValue)) {
                    emptyCellNum ++;
                    continue;
                }
            ...
            if (emptyCellNum != usefulSetMethods.size()){
                result.add(bean);
            }
        }
        return result;

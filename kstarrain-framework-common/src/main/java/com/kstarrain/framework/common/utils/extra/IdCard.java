...
/**
 * Created by liliang on 2016/11/14.
 * 身份证验证信息
 */
public class IdCard {
    ...
    public Date getDateBirthday() {
        String birthdayStr = getBirthday();
    ...
    public int isCorrect() {
        ...
        if (this.is18() && !this.getCheck().equals(this.getCheckBit())) {
    ...
    private boolean isCharCorrect() {
		...
		if (this.is18()) {
			for (int i = 0; i < temp.length; i++) {
				if ((temp[i] < 48 || temp[i] > 57) && (i == 17 && temp[i] != 88)) {
					iRet = false;
					break;
				}
			}
    ...
    public Integer getAge() {
        ...
        /** 年月日 */
        int year = Integer.parseInt(idCardNum.substring(6, 10)); // sonar建议用parseInt增加效率
        int month = Integer.parseInt(idCardNum.substring(10, 12));
        int day = Integer.parseInt(idCardNum.substring(12, 14));
        Calendar bef = Calendar.getInstance();
        Calendar aft = Calendar.getInstance();
        bef.set(year, month, day, 0, 0, 0);
        aft.setTime(new Date());
        int yearNow = aft.get(Calendar.YEAR);
        int monthNow = aft.get(Calendar.MONTH) + 1;
        int dayOfMonthNow = aft.get(Calendar.DAY_OF_MONTH);
